...
export function IncomeOutcomeChart({ data, loading }: IncomeOutcomeChartProps) {
  if (loading) {
    return (
      <Card className="border-border/60" aria-busy="true" role="status">
        <span className="sr-only">Cargando gráfico de ingresos y gastos</span>
  ...
  const descriptionId = "income-outcome-chart-description"
  const titleId = "income-outcome-chart-title"
        ...
        {!hasData ? (
          ...
          <>
            ...
            <p id={descriptionId} className="sr-only">
              {data.map((point) => `${point.month}: ingresos ${formatCurrency(point.income)}, gastos ${formatCurrency(point.outcome)}.`).join(" ")}
            </p>
          </>
        )}
      </CardContent>
