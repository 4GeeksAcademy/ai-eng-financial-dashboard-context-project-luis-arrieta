...
The frontend uses the Vite proxy for `/api` by default. Docker Compose configures its proxy target through `VITE_API_PROXY_TARGET` so the frontend container can reach the backend through the Docker host gateway.
